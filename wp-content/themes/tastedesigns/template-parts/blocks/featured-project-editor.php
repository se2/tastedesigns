...
if ($project) :
  ...
  while (have_rows('colors')) : the_row();
    ...
    if ($override) :
      $colors = array_fill_keys(array('background', 'featured', 'location', 'title', 'body', 'featured_partners', 'partner_title', 'project_detail', 'all_projects'), '');
      foreach ($colors as $key => $value) :
        $colors[$key] = (get_sub_field('color_'.$key) != 'other') ? get_sub_field('color_'.$key) : get_sub_field('color_'.$key.'_alternative');
      endforeach;
    endif;
  endwhile;
?>
  ...
  >
    <div class="c-featured-project__inner flex flex-col lg:flex-row">
      <div class="c-featured-project__gallery w-full lg:w-66% order lg:pl-40">
        ...
        <?php if ($override) : ?>
          style="color:<?php echo $colors['featured']; ?>;"
        <?php endif; ?>
        >
          Featured Project
        </h2>
      ...
      <div class="c-featured-project__content w-full lg:w-34% px-20 lg:px-50">
        ...
        <?php if (get_field('location')) : ?>
          ...
          <?php if ($override) : ?>
            style="color:<?php echo $colors['location']; ?>;"
          <?php endif; ?>
          >
        ...
        <?php if ($override) : ?>
          style="color:<?php echo $colors['title']; ?>;"
        <?php endif; ?>
        >
          <?php the_title(); ?>
        </h1>
        ...
        <?php if ($override) : ?>
          style="color:<?php echo $colors['body']; ?>;"
        <?php endif; ?>
        >
          <?php the_excerpt(); ?>
        </div>
        ...
        <?php if (have_rows('partners')) : ?>
          ...
          <?php if ($override) : ?>
            style="color:<?php echo $colors['featured_partners']; ?>;"
          <?php endif; ?>
          >
            Project Partners
          </h3>
          <?php while (have_rows('partners')) : the_row(); ?>
            ...
            <?php if ($override) : ?>
              style="color:<?php echo $colors['partner_title']; ?>;"
            <?php endif; ?>
            >
    ...
    <div class="c-featured-project__bottom w-full px-20 lg:px-50 mt-50 flex items-center flex-col lg:flex-row">
      <a href="<?php echo get_permalink($project); ?>" class="no-underline w-full lg:w-auto">
        <div class="c-featured-project__button border-2 border-taste-4 py-25 px-21 items-center inline-flex w-full lg:w-auto">
          ...
          <?php if ($override) : ?>
            style="color:<?php echo $colors['project_detail']; ?>;"
          <?php endif; ?>
          >
      ...
      <a href="<?php echo get_post_type_archive_link('project'); ?>" class="c-featured-project__link border-b-4 no-underline mt-50 lg:mt-0 mx-auto lg:mr-0 lg:ml-auto pb-5 lg:pb-0">
        <span class="text-14 leading-17 tracking-3.71 lg:text-18 lg:leading-21 lg:tracking-3 text-taste-2 uppercase"
        <?php if ($override) : ?>
          style="color:<?php echo $colors['all_projects']; ?>;"
        <?php endif; ?>
        >
          View All Projects
        </span>
