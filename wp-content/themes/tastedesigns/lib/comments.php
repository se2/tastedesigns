...
class Comment_Walker extends Walker_Comment {
  ...
  function start_el( &$output, $comment, $depth = 0, $args = array(), $id = 0 ) {
    ...
    <article <?php comment_class((empty( $args['has_children'] ) ? '' : 'parent')); ?> id="comment-<?php comment_ID() ?>" itemprop="comment" itemscope itemtype="http://schema.org/Comment">
      <div class="comment-content-inner flex p-20 bg-taste-8 my-20 lg:my-40">
        ...
        <div class="comment-content pl-40">
          <div class="comment-body min-h-87 pt-10">
            <div class="comment-meta post-meta" role="complementary">
              <h2 class="comment-author font-body text-14 leading-21 lg:text-16 lg:leading-24 text-taste-1 font-bold">
              ...
              <p class="comment-meta-item font-body text-14 leading-21 lg:text-16 lg:leading-24 text-taste-3 italic">Your comment is awaiting moderation.</p>
              <?php endif; ?>
            </div>
            <div class="comment-content post-content font-body text-14 leading-21 lg:text-16 lg:leading-24 text-taste-1" itemprop="text">
              <?php comment_text(); ?>
            </div>
          </div>
...
add_action('after_setup_theme', function() {
  ...
  add_filter('comment_form_default_fields', function($fields) {
    ...
    $fields['author'] = '<div class="w-half pr-8 lg:pr-25 inline-block"><input id="author" class="font-body text-14 leading-17 text-taste-2 border-2 border-taste-4 px-20 py-20 w-full" name="author" type="text" placeholder="First Name" size="30" tabindex="1"' . $aria_req . ' /></div>';

    $fields['last_name'] = '<div class="w-half pl-8 lg:pl-25 inline-block"><input id="last-name" class="font-body text-14 leading-17 text-taste-2 border-2 border-taste-4 px-20 py-20 w-full" name="last-name" type="text" placeholder="Last Name" size="30" tabindex="2"' . $aria_req . ' /></div>';
